...
///
/// Uses RwLock for efficient concurrent read access with exclusive write access.
...
pub struct SettingsCache {
    /// Full settings JSON value
    settings: RwLock<Option<Value>>,
    /// Cached: voiceInput.microphoneDeviceId
    microphone_device_id: RwLock<Option<String>>,
    /// Cached: system.playSoundOnRecording
    play_sound_on_recording: RwLock<Option<bool>>,
    /// Cached: voiceInput.shortcut
    voice_input_shortcut: RwLock<Option<String>>,
    /// Cached: voiceInput.pushToTalkShortcut
    ptt_shortcut: RwLock<Option<String>>,
    /// Cached: voiceInput.enablePushToTalk
    enable_push_to_talk: RwLock<Option<bool>>,
    /// Cached: shortcuts.pasteLastTranscript
    paste_shortcut: RwLock<Option<String>>,
    /// Cached: shortcuts.globalShortcutsEnabled
...
impl SettingsCache {
    ...
    fn update_from_value(&self, settings: &Value) {
        ...
        if let Some(obj) = settings.as_object() {
            ...
            if let Some(voice_input) = obj.get("voiceInput").and_then(|v| v.as_object()) {
                if let Ok(mut cache) = self.microphone_device_id.write() {
                    *cache = voice_input
                        .get("microphoneDeviceId")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string());
                }
                if let Ok(mut cache) = self.voice_input_shortcut.write() {
                    *cache = voice_input
                        .get("shortcut")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string());
                }
                if let Ok(mut cache) = self.ptt_shortcut.write() {
                    *cache = voice_input
                        .get("pushToTalkShortcut")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string());
                }
                if let Ok(mut cache) = self.enable_push_to_talk.write() {
                    *cache = voice_input
            ...
            if let Some(system) = obj.get("system").and_then(|v| v.as_object()) {
                if let Ok(mut cache) = self.play_sound_on_recording.write() {
                    *cache = system.get("playSoundOnRecording").and_then(|v| v.as_bool());
                }
            }

            // shortcuts settings
            if let Some(shortcuts) = obj.get("shortcuts").and_then(|v| v.as_object()) {
                if let Ok(mut cache) = self.paste_shortcut.write() {
                    *cache = shortcuts
                        .get("pasteLastTranscript")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string());
                }
                if let Ok(mut cache) = self.global_shortcuts_enabled.write() {
                    *cache = shortcuts
    ...
    pub fn get_settings(&self) -> Option<Value> {
        self.settings.read().ok().and_then(|guard| guard.clone())
    }
